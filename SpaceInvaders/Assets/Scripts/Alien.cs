...
public class Alien : MonoBehaviour
{
    ...
    public Sprite explodedShiptImage;
    ...
    public IEnumerator changeAlienSprite()
    {
        while (true)
        {
            if (spriteRenderer.sprite == startingImage)
            {
                ...
                //gives somekind of bug
            }
            else
            {
                spriteRenderer.sprite = startingImage;
                //SoundManager.Instance.PlayOneShot(SoundManager.Instance.alienBuzz2);
                // gives bug it's trying to make an instance of soundmanger and play
                // the sound but the problem is that there is no object created yet
            }
    ...
    void OnTriggerEnter2D(Collider2D col)
    {
        if (col.gameObject.tag == "Player")
        {
            ...
            col.GetComponent<SpriteRenderer>().sprite = explodedShiptImage;
            Destroy(gameObject);
